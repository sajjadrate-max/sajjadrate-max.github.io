...
function isAllowedOrigin(req) {
  ...
  if (!origin) return false; // block requests with no origin/referer at all
  try {
...
export default async function handler(req, res) {
  ...
  // 1) Block requests not coming from your own website
  if (!isAllowedOrigin(req)) {
    return res.status(403).json({ error: "Forbidden" });
  }

  // 2) Basic per-IP rate limiting
  ...
  try {
    ...
    // 3) Basic length guard so a single request can't burn huge amounts of tokens
